...
axiosClient.interceptors.response.use(
	...
	async (error) => {
		...
		if (originalConfig.url === 'e-commerce/auth/login' || originalConfig.url === 'auth/revoke-access-token') {
			return Promise.reject(error)
		}
		...
		try {
			if (!getNewToken) {
				getNewToken = (async () => {
					...
					return await axiosClient.post('auth/revoke-access-token', { refresh_token: refreshToken })
				})()
			}

			const refreshTokenResponse = await getNewToken
			originalConfig.headers.Authorization = `Bearer ${refreshTokenResponse.data.data.access_token}`

			// console.log(">>", refreshTokenResponse)

			LocalStorageService.setTokenData(refreshTokenResponse.data.data)
